...
import { usePendingApprovals, useSettlementQueue, useDashboardStats } from '@/services';
...
const QuickLinks = () => {
  ...
  const { data: dashboardStats, isLoading: isLoadingStats } = useDashboardStats({ period: 'today' });
  const { data: pendingApprovals, isLoading: isLoadingApprovals } = usePendingApprovals();
  const { data: settlementQueue, isLoading: isLoadingSettlements } = useSettlementQueue();
  ...
  const quickLinks: IQuickLink[] = [
    {
      ...
      color: 'primary',
      badge: {
        text: isLoadingStats ? '...' : (dashboardStats?.bookings?.total || 0),
        variant: 'primary'
      }
    },
    {
      ...
      color: 'warning',
      badge: {
        text: isLoadingApprovals ? '...' : (pendingApprovals || 0),
        variant: 'warning'
      }
    },
    {
      ...
      icon: 'wallet',
      path: '/admin/payments?tab=provider-payouts&status=pending',
      color: 'success',
      badge: {
        text: isLoadingSettlements ? '...' : (settlementQueue || 0),
        variant: 'success'
      }
    }
  ];
      ...
      >
        <div className="card-body">
          <div className="flex items-start justify-between mb-4">
            <div className="p-3 rounded-lg bg-white/50 group-hover:bg-white/20 transition-colors flex items-center justify-center">
              <KeenIcon icon={link.icon} className="text-xl text-current" />
            </div>
            {link.badge && (
