  ...
  const fetchUserData = useCallback(async (userId: string) => {
    ...
    try {
      ...
      if (paymentsData?.data?.transactions) {
        paymentsList = Array.isArray(paymentsData.data.transactions)
          ? paymentsData.data.transactions
          : [];
  ...
  if (userDetailsLoading) {
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="flex min-h-[400px] max-w-4xl items-center justify-center">
          <ContentLoader />
        </DialogContent>
      </Dialog>
    );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-5xl max-h-[92vh] overflow-hidden p-0">
        ...
        <div className="max-h-[calc(92vh-136px)] overflow-y-auto px-6 pb-6">
          <Tabs defaultValue="profile" className="w-full">
            <TabsList className="flex w-full gap-3 overflow-x-auto whitespace-nowrap px-1 py-2">
              <Tab value="profile" className="shrink-0 px-4 py-2">
                Profile
              </Tab>
              <Tab value="bookings" className="shrink-0 px-4 py-2">
                Bookings
              </Tab>
              <Tab value="payments" className="shrink-0 px-4 py-2">
                Payments
              </Tab>
              <Tab value="tickets" className="shrink-0 px-4 py-2">
                Support
              </Tab>
            </TabsList>
                  ...
                  <div className="card-body space-y-4">
                    <div className="flex items-center gap-4">
                      {displayUser.profile_picture_url ? (
                        <img
                          src={
                            displayUser.profile_picture_url.startsWith('http')
                              ? displayUser.profile_picture_url
                      ...
                      <div
                        className={`w-16 h-16 bg-primary-light rounded-full flex items-center justify-center ${displayUser.profile_picture_url ? 'hidden' : ''}`}
                      >
                        ...
                        <p className="text-muted-foreground">
                          {(() => {
                            ...
                            let cleanPhone = phone
                              .replace(
                                new RegExp(`^${countryCode.replace('+', '\\+')}\\s*`, 'g'),
                                ''
                              )
                              .trim();
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <div>
                        ...
                        <p className="text-sm text-foreground">
                          {displayUser.user_id || displayUser.id}
                        </p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-foreground">
                          ...
                          displayUser.last_login_at !== null &&
                          displayUser.last_login_at !== undefined
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="rounded-lg bg-primary-light p-4 text-center">
                        <div className="text-2xl font-bold text-primary">
                          {displayUser.stats?.total_orders || displayUser.totalBookings || 0}
                        </div>
                        <div className="text-sm text-muted-foreground">Total Bookings</div>
                      </div>
                      <div className="rounded-lg bg-success-light p-4 text-center">
                        <div className="text-2xl font-bold text-success">
                          ₹{displayUser.stats?.total_revenue || displayUser.totalSpent || 0}
                        </div>
                        ...
                        {bookingsLoading ? (
                          ...
                          bookings.map((booking: any) => {
                            ...
                            const providerName =
                              ...
                              (booking.provider
                                ? `${booking.provider.business_name || booking.provider.contact_name || 'N/A'}`
                                : 'N/A');
                            const bookingDate =
                              booking.scheduled_date || booking.created_at || booking.date;
                      ...
                      <TableBody>
                        {paymentsLoading ? (
                          ...
                          payments.map((payment: any) => {
                            ...
                            const paymentId =
                              payment.transaction_id || payment.public_id || payment.id;
                            ...
                            const method =
                              ...
                              'N/A';
                            const paymentDate =
                              payment.created_at || payment.completed_at || payment.date;
                          ...
                          supportTickets.map((ticket: any) => (
                            <TableRow key={ticket.dispute_id || ticket.public_id || ticket.id}>
                              <TableCell className="font-medium">
                                {ticket.dispute_id || ticket.public_id || ticket.id}
                              </TableCell>
                              <TableCell>
                                <div>
                                  ...
                                  <div className="text-sm text-muted-foreground">
                                    {ticket.description || ticket.issue_description || ''}
                                  </div>
                                </div>
                              </TableCell>
                              <TableCell>
                                {getPriorityBadge(ticket.priority?.toLowerCase() || 'medium')}
                              </TableCell>
                              <TableCell>
                                {ticket.created_at
                              ...
                              <TableCell>
                                {getStatusBadge(ticket.status?.toLowerCase() || 'open')}
                              </TableCell>
                            </TableRow>
                          ))
                        )}
